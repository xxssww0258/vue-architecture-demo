...
const webAxios = axios.create({
    timeout: 5000,
    validateStatus (status) { // 如果接口返回符合如下就返回resolve否则reject
...
// ---------------------------------------- 请求处理 ----------------------------------------
webAxios.interceptors.request.use(function (config) {
    // 在发送请求之前做些什么
    return config
}, function (error) {
    // 对请求错误做些什么
...
// ---------------------------------------- 相应处理 ----------------------------------------
webAxios.interceptors.response.use(function (response) {
    let data = response.data
    // 这里用来统一 不同的后端返回的数据类型
    // 有些后端喜欢返回错 data.errCode  有些喜欢返回 http.status
    if (data.isSuccess === 1) { return Promise.reject('error') }
